...
# To bias toward "more random among genres" (uniform tag round-robin) and reduce artist clustering:
python random_mb_to_spotify.py --n 1000 --name "Global Mix" --artist-cap 2
# To customize the genre tag set:
python random_mb_to_spotify.py --n 1000 --name "Custom Mix" --genres "pop,hip hop,edm,afrobeats,latin,metal,jazz,classical"
"""
...
MB_APP_NAME = "RandomMBToSpotify/1.1 (contact: you@example.com)"  # replace with your contact
...
def mb_round_robin_by_tags(tags: List[str], batch_per_tag: int = 20) -> Iterable[dict]:
    """
    Yield recordings by cycling tags; from each tag page, emit up to batch_per_tag items
    that pass lightweight filters, then move to next tag.
    """
    tags = [normalize_tag_name(t) for t in tags if t.strip()]
    if not tags:
        tags = DEFAULT_GENRE_TAGS[:]

    # randomize the order of tags each full cycle to reduce any residual bias
    for tag in itertools.cycle(random.sample(tags, k=len(tags))):
        try:
            recs = mb_fetch_by_tag(tag)
        except Exception:
            continue  # skip this turn on transient error
        emitted = 0
        for r in recs:
            length = r.get("length")
...
def mb_sample_recordings(batch_target: int) -> Iterable[dict]:
    ...
    Left for reference; the round-robin sampler is the default now.
    """
    page_size = 100
    tags = DEFAULT_GENRE_TAGS
    while True:
...
def create_playlist(sp: spotipy.Spotify, name: str, public: bool) -> Tuple[str, str]:
    ...
    pl = sp.user_playlist_create(
        ...
        description="True-random, genre-rotating sample via MusicBrainz"
    )
...
def build_true_random_playlist(n: int,
    ...
    for r in rr_iter:
        ...
        # Enforce artist cap
        if primary_artist:
            if per_artist_count.get(primary_artist, 0) >= artist_cap:
                continue
