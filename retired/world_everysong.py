...
MB_APP_NAME = "RandomMBToSpotify/1.2 (contact: you@example.com)"  # replace with your contact
...
DEFAULT_GENRE_TAGS = [
    "pop","hip hop","rap","trap","r&b","soul","afrobeats","afrobeat","amapiano",
    "latin","reggaeton","regional mexican","corrido","banda","salsa","bachata",
    "k-pop","j-pop","city pop","cantopop","indie pop","indie rock","alt-rock",
    "edm","electronic","house","deep house","progressive house","tech house",
    "techno","hard techno","trance","psytrance","drum and bass","dubstep","garage",
    "grime","uk drill","breakbeat","lo-fi","chillout","ambient","synthwave","hyperpop",
    "rock","classic rock","punk","post-punk","shoegaze","emo","post-rock","metal","metalcore",
    "country","folk","bluegrass","blues","jazz","fusion","bossa nova","samba","mpb",
    "reggae","dancehall","ska","gospel","soundtrack","classical","contemporary classical","opera"
]
...
DEFAULT_MARKETS_GLOBAL = [
    ...
    "GB","IE","DE","FR","NL","BE","LU","CH","AT","IT","ES","PT","SE","NO","DK","FI","PL","CZ","RO","HU","GR","TR","UA",
    "RU",  # may be restricted for some accounts; harmless if skipped by API
    "AU","NZ",
    "JP","KR","CN",  # CN not a Spotify market; will be skipped—kept here if you remove it, no harm.
    "HK","TW","SG","MY","TH","VN","PH","ID","IN",
    "AE","SA","EG","ZA","NG","KE","MA","IL"
]
# Remove invalids defensively (Spotify ignores CN; we’ll filter it out later).
...
# 3) Updated sampler: add year_min / year_max filters (inclusive)
def mb_round_robin_by_tags(
    ...
    for tag in itertools.cycle(random.sample(tags, k=len(tags))):
        ...
        for r in recs:
            ...
            # year filter (inclusive). If filtering is requested and year is unknown, skip.
...
def resolve_spotify_track_global(
    ...
    """
    markets = [m for m in markets if m and m.upper() != "CN"]  # CN isn't a Spotify market
    if not markets:
        markets = ["US"]
...
def build_true_random_playlist(n: int,
                               name: str,
                               public: bool,
                               genres_csv: str,
                               per_genre_batch: int,
                               artist_cap: int,
                               markets_csv: Optional[str]) -> str:
    sp = spotify_client()
    ...
    print(f"[INFO] Resolving across markets: {', '.join(markets[:12])}{' ...' if len(markets)>12 else ''}")
    ...
    # Round-robin generator across tags
    rr_iter = mb_round_robin_by_tags(chosen_tags, batch_per_tag=per_genre_batch)
...
def parse_args() -> argparse.Namespace:
    ...
    # Genre balancing
    ap.add_argument("--genres", type=str, default=",".join(DEFAULT_GENRE_TAGS),
                    help="Comma-separated MusicBrainz tags to sample uniformly across.")
    ap.add_argument("--per-genre-batch", type=int, default=20,
                    help="Candidates to emit per tag each round (default: 20).")
    ap.add_argument("--artist-cap", type=int, default=2,
                    help="Max tracks per primary artist (default: 2).")
    # Markets
    ap.add_argument("--markets", type=str, default=None,
                    help='Comma-separated list of ISO markets to rotate (e.g., "US,GB,BR,JP") or "GLOBAL" for a wide preset. '
                         'If omitted, uses $SPOTIFY_MARKET or US.')
    # 5) Add CLI args in parse_args():
...
def main():
    ...
    for group in (("SPOTIPY_CLIENT_ID", "SPOTIFY_CLIENT_ID"),
